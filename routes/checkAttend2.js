...
function display(req, res){
  ...
  res.render('checkAttend2', { title: 'checkAttend', params: params});
}
...
function getDescriptor(filePath, fileName, stuNums){
  ...
  for(var i=0; i<afterImgs.length; i++){
    ...
    shell.cd(rootDir + '/../face_recognition/src/build/');
    console.log('-----------------' + './check_attendance2 ' + afterImgs[i].replace('.png','.txt') + ' ' + stuNums);
    var file = shell.exec('./check_attendance2 ' + afterImgs[i].replace('.png','.txt') + ' ' + stuNums).stdout;
